        ...
        if (path.value.arguments.length === 2) {
          if (hasFlag('deep') && hasFlag('own')) {
            return j.sequenceExpression([
              ...
              deepEquality(j.memberExpression(subject, matcherArg, true), path.value.arguments[1])
            ]);
          } else if (hasFlag('own')) {
            return assertion(
              j.logicalExpression(
                ...
                j.binaryExpression(
                  negation ? '!==' : '===',
                  j.memberExpression(subject, matcherArg, true),
                  path.value.arguments[1]
                )
              )
            );
          } else if (hasFlag('deep')) {
            return deepEquality(j.memberExpression(subject, matcherArg, true), path.value.arguments[1]);
          } else {
            return strictEquality(j.memberExpression(subject, matcherArg, true), path.value.arguments[1]);
          }
        } else {
          if (hasFlag('own')) {
            return assertion(
